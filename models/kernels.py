...
class InverseDistanceWithParam(gpytorch.kernels.Kernel):
    ...
    def forward(self, x1, x2, mode = 'train',device=device, **params):
        ...
        a[a > 1e-15] = 1   # matrix for LHOCV regularization
        if mode == 'train':
            ...
            return k*torch.ones_like(k).fill_diagonal_(1e-40).to(device)*a
        else:
            k = torch.exp(self.power_param*torch.log(1 / diff + 1e-7  ))
